    ...
    public synchronized void updateOrderEntry(long           id,
        ...
        try {
            ...
            try {
                ...
                stmt.setString(12, ticket);
                stmt.setString(13, note);
                stmt.setInt   (14, (int)offsettingOrder);

                if (dateConsidered == null) {
                    stmt.setNull(15, java.sql.Types.VARCHAR);
                } else {
                    stmt.setString(15, FormatDate.formatDate(dateConsidered, SQL_DATE_FORMAT));
                }

                if (datePlaced == null) {
                    stmt.setNull(16, java.sql.Types.VARCHAR);
                } else {
                    stmt.setString(16, FormatDate.formatDate(datePlaced, SQL_DATE_FORMAT));
                }

                if (dateFilled == null) {
                    stmt.setNull(17, java.sql.Types.VARCHAR);
                } else {
                    stmt.setString(17, FormatDate.formatDate(dateFilled, SQL_DATE_FORMAT));
                }

                if (dateCancelPlaced == null) {
                    stmt.setNull(18, java.sql.Types.VARCHAR);
                } else {
                    stmt.setString(18, FormatDate.formatDate(dateCancelPlaced, SQL_DATE_FORMAT));
                }

                if (dateCancelled == null) {
                    stmt.setNull(19, java.sql.Types.VARCHAR);
                } else {
                    stmt.setString(19, FormatDate.formatDate(dateCancelled, SQL_DATE_FORMAT));
                }

                if (dateStopLoss == null) {
                    stmt.setNull(20, java.sql.Types.VARCHAR);
                } else {
                    stmt.setString(20, FormatDate.formatDate(dateStopLoss, SQL_DATE_FORMAT));
                }
