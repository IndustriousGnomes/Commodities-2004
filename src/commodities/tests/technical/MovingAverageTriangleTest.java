...
/**
 *  The MovingAverageTriangleTest performs the moving average test using exponential
...
public class MovingAverageTriangleTest extends TechnicalTestAbstract {
/* *************************************** */
/* *** CLASS VARIABLES                 *** */
/* *************************************** */
    /** The name of the test. */
    private static final String TEST_NAME = "Moving Average - Triangle";
    /** The name of the sql table associated with this test. */
    private static final String SQL_TABLE = "test_moving_averages_triangle";
...
/* *************************************** */
    ...
    /* *************************************** */
        ...
        protected LinkedList getData() {
            if (movingAverage == null) {
                movingAverage = new MovingAverageTriangle(interval, sourceMap);
    ...
    /* *************************************** */
        ...
        public GraphPanel(boolean resetCount) {
            ...
            graphNbr = graphCount++;
System.out.println("MovingAverageTriangleTest - adding graph listener");
...
/* *************************************** */
     ...
     *  methods.
     */
    public MovingAverageTriangleTest() {
        testName = TEST_NAME;
        sqlTable = SQL_TABLE;
    }
     ...
     */
    public MovingAverageTriangleTest(Contract contract) {
        this();
...
// recalculate the statististics data by adding it to the queue for that moving average.
    ...
    protected void loadParameters() {
        ...
        try {
            ...
            while (rs.next()) {
                int id = rs.getInt(1);
                int paramDays = rs.getInt(2);
                Stats stats = new Stats(paramDays);
