   ...
   public synchronized long getLastOrderIndex() throws IOException {
    ...
    public synchronized void updateOrderEntry(long           id,
        ...
        try {
            ...
            preparedUpdateOrderEntry.setString(11, ticket);
            preparedUpdateOrderEntry.setString(12, note);
            preparedUpdateOrderEntry.setLong  (13, offsettingOrder);

            if (dateConsidered == null) {
                preparedUpdateOrderEntry.setNull(14, java.sql.Types.DATE);
            } else {
                preparedUpdateOrderEntry.setDate(14, new java.sql.Date(dateConsidered.getTime()));
            }

            if (datePlaced == null) {
                preparedUpdateOrderEntry.setNull(15, java.sql.Types.DATE);
            } else {
                preparedUpdateOrderEntry.setDate(15, new java.sql.Date(datePlaced.getTime()));
            }

            if (dateFilled == null) {
                preparedUpdateOrderEntry.setNull(16, java.sql.Types.DATE);
            } else {
                preparedUpdateOrderEntry.setDate(16, new java.sql.Date(dateFilled.getTime()));
            }

            if (dateCancelPlaced == null) {
                preparedUpdateOrderEntry.setNull(17, java.sql.Types.DATE);
            } else {
                preparedUpdateOrderEntry.setDate(17, new java.sql.Date(dateCancelPlaced.getTime()));
            }

            if (dateCancelled == null) {
                preparedUpdateOrderEntry.setNull(18, java.sql.Types.DATE);
            } else {
                preparedUpdateOrderEntry.setDate(18, new java.sql.Date(dateCancelled.getTime()));
            }

            if (dateStopLoss == null) {
                preparedUpdateOrderEntry.setNull(19, java.sql.Types.DATE);
            } else {
                preparedUpdateOrderEntry.setDate(19, new java.sql.Date(dateStopLoss.getTime()));
            }



            preparedUpdateOrderEntry.setLong  (20, id);
            ...
            if (count == 0) {
             ...
             try {
                ...
                preparedInsertOrderEntry.setString(12, ticket);
                preparedInsertOrderEntry.setString(13, note);
                preparedInsertOrderEntry.setLong  (14, offsettingOrder);

                if (dateConsidered == null) {
                    preparedInsertOrderEntry.setNull(15, java.sql.Types.DATE);
                } else {
                    preparedInsertOrderEntry.setDate(15, new java.sql.Date(dateConsidered.getTime()));
                }

                if (datePlaced == null) {
                    preparedInsertOrderEntry.setNull(16, java.sql.Types.DATE);
                } else {
                    preparedInsertOrderEntry.setDate(16, new java.sql.Date(datePlaced.getTime()));
                }

                if (dateFilled == null) {
                    preparedInsertOrderEntry.setNull(17, java.sql.Types.DATE);
                } else {
                    preparedInsertOrderEntry.setDate(17, new java.sql.Date(dateFilled.getTime()));
                }

                if (dateCancelPlaced == null) {
                    preparedInsertOrderEntry.setNull(18, java.sql.Types.DATE);
                } else {
                    preparedInsertOrderEntry.setDate(18, new java.sql.Date(dateCancelPlaced.getTime()));
                }

                if (dateCancelled == null) {
                    preparedInsertOrderEntry.setNull(19, java.sql.Types.DATE);
                } else {
                    preparedInsertOrderEntry.setDate(19, new java.sql.Date(dateCancelled.getTime()));
                }

                if (dateStopLoss == null) {
                    preparedInsertOrderEntry.setNull(20, java.sql.Types.DATE);
                } else {
                    preparedInsertOrderEntry.setDate(20, new java.sql.Date(dateStopLoss.getTime()));
                }
