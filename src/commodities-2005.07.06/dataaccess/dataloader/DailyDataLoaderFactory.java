...
/* *** INSTANCE VARIABLES              *** */
/* *************************************** */
    /** Daily price loading is complete. */
    private boolean loadingComplete = false;

    /** Listeners for changing prices. */
    private Collection loadedListeners = new HashSet();
...
//        cme.retrievePrices();
    ...
    /**
     *  Indicates if the daily data has completed loading.
     *
     *  @return true if daily price data has completed loading.
     */
...
/* ***---------------------------------*** */
    /**
     *  Add a listener for when the daily data is loaded.
     *
     *  @param  listener    a DailyDataLoadedListener
     */
    public void addListener(DailyDataLoadedListener listener) {
        if (!loadedListeners.contains(listener)) {
            loadedListeners.add(listener);
            if (isLoadingComplete()) {
                fireDailyDataLoaded(listener);
            }
        }
    }
     ...
     *  @param  listener    a DailyDataLoadedListener
     */
    public void removeListener(DailyDataLoadedListener listener) {
    ...
    /**
     *  Fire daily data loaded information to all DailyDataLoadedListeners.
     */
    private void fireDailyDataLoaded() {
        DailyDataLoadedEvent event = new DailyDataLoadedEvent(this);

        Iterator it = loadedListeners.iterator();
        while (it.hasNext()) {
            DailyDataLoadedListener listener = (DailyDataLoadedListener)it.next();
            listener.dailyDataLoaded(event);
        }
    }

    /**
     *  Fire daily data loaded information to the single DailyDataLoadedListener.
     *
     *  @param  listener    a DailyDataLoadedListener
     */
    private void fireDailyDataLoaded(DailyDataLoadedListener listener) {
        DailyDataLoadedEvent event = new DailyDataLoadedEvent(this);
        listener.dailyDataLoaded(event);
    }
