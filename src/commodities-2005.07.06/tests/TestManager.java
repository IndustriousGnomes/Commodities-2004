...
public class TestManager implements DailyDataLoadedListener, PriceListener, ContractSelectionListener {
...
//    private Set recommendSet = new HashSet();
    ...
    /** A lock to use for synchronization. */
    private Object syncLock = new Object();
    /** Indicator that daily data loading is complete. */
...
*/
        loadTests();
        DailyDataLoaderFactory.instance().addListener(this);
    }
...
/* *************************************** */
     ...
     */
    private synchronized TechnicalTestInterface getTestInstance(String testName, Contract contract) throws Exception {
    ...
    /**
     *  Get the recommendation for the desired test on the given contract and date.
     *
     *  @param  testName    name of the test (from menu)
     *  @param  contract    contract to get the recommendation for
     *  @param  testId      the test id to get the recommendation for
     *  @param  date        the date to get the recommendation for
     */
    public Recommendation getTestRecommendation(String testName, Contract contract,
                                                int testId, java.util.Date date) throws Exception {
        if (!dataLoaded) {
        ...
        return test.getRecommendation(testId, date);
    }

    /**
     *  Perform the optimization for the desired test on the given contract.
     *
     *  @param  testName    name of the test (from menu)
     *  @param  contract    contract to get the recommendation for
     */
    public void optimizeTest(String testName, Contract contract) throws Exception {
        if (!dataLoaded) {
            synchronized (syncLock) {
                while (!dataLoaded) {
                    try {
...
/* *************************************** */
/* *** DailyDataLoadedListener METHODS *** */
/* *************************************** */
    /**
    ...
    public void dailyDataLoaded(DailyDataLoadedEvent e) {
        dataLoaded = true;
/*
        Iterator it = testsByContract.values().iterator();
        while (it.hasNext()) {
            tableOfTests = (HashMap)it.next();
            Iterator it2 = tableOfTests.values().iterator();
            while (it2.hasNext()) {
                ((TechnicalTestInterface)it2.next()).loadData();
            }
        }
*/
    }
...
/*
        if (DailyDataLoaderFactory.instance().isLoadingComplete()) {
            Iterator it = testsByContract.values().iterator();
            while (it.hasNext()) {
                tableOfTests = (HashMap)it.next();
                Iterator it2 = tableOfTests.values().iterator();
