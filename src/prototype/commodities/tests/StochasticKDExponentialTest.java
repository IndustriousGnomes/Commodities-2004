...
/**
 *  The StochasticKDExponentialTest performs the stochastic oscillator test where
 *  the oscillator (%K) is compared to a exponential moving average (%D) of the
 *  oscillator.  When %K crosses %D, a buy/sell signal is produced.
 *
 *  @author J.R. Titko
 */
public class StochasticKDExponentialTest extends TechnicalTestAbstract {
...
/* *************************************** */
    ...
    private static final String TEST_NAME = "Stochastic %K/%D (exponential)";
    /** The name of the sql table associated with this test. */
    private static final String SQL_TABLE = "test_stochastic_kd_exponential";
...
/* *************************************** */
    ...
    /* *************************************** */
        ...
        /** The number of time periods for %D exponential moving average. */
        protected int dPercent;

        /** Source data for determining the moving average. */
        private TreeMap sourceMap = new TreeMap();
        /** Instance of MovingAverage to use. */
        private MovingAverageExponential dMovingAverage;

    /* *************************************** */
    /* *** CONSTRUCTORS                    *** */
    /* *************************************** */
        public Stats(int kPeriods, int kSlowing, int dPercent) {
    ...
    /* *************************************** */
        ...
        public void calculateStatData() {
            ...
            dMovingAverage = new MovingAverageExponential(dPercent, kMap);
    ...
    /* *************************************** */
        ...
        protected String getName() {
            ...
            return "" + kPeriods + "(" + speed + "w/ %D=" + dPercent + ")";
        }
    ...
    /* *************************************** */
        ...
        public int compareTo(Object o2) {
            ...
            if (c == 0) {
                c = dPercent - ((Stats)o2).getDPercent();
            }
...
Debug.println(DEBUG, this, "StochasticKDExponentialTest.GraphPanel.getYBuffer() only");
            return 0;
        }
...
Debug.println(DEBUG, this, "StochasticKDExponentialTest.GraphPanel.getMaxValue() only");
            return 100.0;
        }
...
Debug.println(DEBUG, this, "StochasticKDExponentialTest.GraphPanel.getMinValue() only");
            return 0.0;
        }
...
Debug.println(DEBUG, this, "StochasticKDExponentialTest.GraphPanel.getTickSize() only");
            return 1;
        }
...
Debug.println(DEBUG, this, "StochasticTest.GraphPanel.redrawComponent() end");
        ...
        public void drawGraph(Graphics      g,
                              ...
                              double        minValue,
                              double        scale) {
System.out.println("StochasticKDExponentialTest - drawGraph - start");
...
/* *************************************** */
     ...
     *  methods.
     */
    public StochasticKDExponentialTest() {
        testName = TEST_NAME;
        sqlTable = SQL_TABLE;
    }
     ...
     */
    public StochasticKDExponentialTest(Contract contract) {
        this();
...
Debug.println(DEBUG, this, "loadParameters() start");
        ...
        try {
            ...
            sb.append("select id, k_periods, k_slowing_periods, d_percent " +
                      "from " + sqlTable + " " +
...
Debug.println(DEBUG, this, "saveParameters(paramDays) start");
        ...
        String columns = "k_periods, k_slowing_periods, d_percent";
...
Debug.println(DEBUG, this, "graphResults() start");
        ...
        while (it.hasNext()) {
System.out.println("StochasticKDExponentialTest - graphResults() - adding graph");
