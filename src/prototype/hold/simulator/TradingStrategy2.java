...
/**
 *  The TradingStrategy2 simulates the daily actions of the commodity
...
//    private static final int OPTIMIZATION_PERIOD_MONTHS = 1;
    ...
    private static final String REPORT_FILE = "D:\\JavaProjects\\Commodities\\SimulatorReports\\Simulator2Report";
...
/* *************************************** */
     ...
     */
    public TradingStrategy2(LinkedList contracts) {
...
Debug.println(DEBUG, this, "sell stoploss = " + (purchasePrice + calculateStoplossAmount(contract)));
     ...
     *  Determine if a stoploss order needs to be fulfilled.
     */
    private void setNewStopLosses(Contract         contract,
                                  java.util.Date   today,
                                  Map              openOrders,
                                  Map              filledOrders) {
...
Debug.println(DEBUG, this, "adjusting stoploss");

        double closePrice = contract.getPrices(today).getClose();
        if (closePrice == 0.0) {
            closePrice = ((contract.getPrices(today).getHigh() + contract.getPrices(today).getLow()) / 2.0);
        }

        while (it.hasNext()) {
            Order order = (Order)it.next();
            if (order.isBuy()) {
                double newStoploss = (closePrice - calculateStoplossAmount(contract));
                if (order.getStoploss() < newStoploss) {
...
Debug.println(DEBUG, this, "clearSimulatorOrders() start");
        ...
        try {
            ...
            sb.append("delete from Simulator_Orders where simulator_run = 2");
